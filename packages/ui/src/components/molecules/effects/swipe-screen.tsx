    ...
    useEffect(() => {
        ...
        const handleTouchMove = (e: TouchEvent) => {
            if (
                ...
                if (Math.abs(deltaX) > Math.abs(e.targetTouches[0]!.clientY - touchStartX)) {
                    setIsScrollingY(false);
        ...
        const handleTouchEnd = () => {
            ...
            if (swipePercentage > swipeThreshold) {
                if (distance > 0) {
                    controls.start({ x: "100vw", opacity: 0.5,  transition: { type: "spring", stiffness: 500, damping: 30 } }).then(() => {
                        handlePrevPage();
                    });
                } else {
                    controls.start({ x: "-100vw", opacity: 0.5, transition: { type: "spring", stiffness: 500, damping: 30 } }).then(() => {
                ...
                const timeoutId = window.setTimeout(() => {
                    controls.start({ x: 0, opacity: 1, transition: { duration: 0.2 } }).then(() => {
                        window.setTimeout(() => {
                            document.body.style.overflowY = "auto";
                            const container = document.getElementById("container");
                            if (container) {
