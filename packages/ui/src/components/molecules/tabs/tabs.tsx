        ...
        {isLoading ? (
          ...
          >
            <div
              ref={tabsRef}
              className={cn(
                "flex flex-row items-center justify-start [perspective:1000px] sticky top-0 bg-white rounded-sm py-1 md:py-0 md:bg-transparent z-[999] flex-nowrap no-visible-scrollbar max-w-full w-full pl-1 overflow-x-auto",
                containerClassName,
              )}
            >
              {/* //More */}
              ...
              {propTabs.map((tab, idx) => (
                <m.button
                  ...
                  className={cn(
                    "relative px-3 py-1 rounded-md flex-shrink-0",
                    tabClassName,
                  )}
                  style={{
                ...
                >
                  {currentTabActive?.value === tab.value && (
                    <m.div
                      ...
                      className={cn(
                        "absolute inset-0 bg-white/50 shadow-md rounded-md",
                        activeTabClassName,
                      )}
                    />
                  )}
            ...
            <div
              className="fixed right-0 top-0 w-5 md:w-8 h-12 md:h-10 bg-gradient-to-t from-white via-white opacity-80 rounded-r-md z-[1000] flex items-center justify-center cursor-pointer"
              onClick={() => scrollToTab("right")}
            >
              <ChevronRight className="w-4 h-4 md:w-5 md:h-5 opacity-80" />
            </div>
            <div
              className="fixed left-0 top-0 w-5 md:w-8 h-12 md:h-10 bg-gradient-to-t from-white via-white opacity-80 rounded-l-md z-[1000] flex items-center justify-center cursor-pointer"
...
export const FadeInDiv = ({
  ...
  return (
    <AnimatePresence mode="popLayout">
      <div className="relative w-full min-h-screen md:min-h-full">
        {tabs.map((tab, idx) => (
          <m.div
            key={tab.value}
            layoutId={tab.value}
