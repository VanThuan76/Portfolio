...
export function PopoverTrigger({ children, className }: PopoverTriggerProps) {
  ...
  return (
    <m.button
      ...
      onClick={openPopover}
    >
      <m.span layoutId={`popover-label-${uniqueId}`} className="text-sm">
        {children}
      </m.span>
    </m.button>
  );
...
export function PopoverButton({
  ...
  return (
    <button
      className={cn(
        "flex w-full items-center gap-2 rounded-md px-4 py-2 text-left text-sm hover:bg-zinc-300 dark:hover:bg-zinc-700",
        className,
      )}
      onClick={(e) => {
        onClick && onClick(e);
