...
import { useOpenScreen, useBreakpoint } from "@repo/hooks";
                        ...
                        >
                            <>
                                ...
                                {propTabs.map((tab, idx) => (
                                    <m.button
                                        key={idx}
                                        onMouseEnter={!isMobile ? () => (hoverRef.current = true) : undefined}
                                        onMouseLeave={!isMobile ? () => (hoverRef.current = false) : undefined}
                                        onClick={() => {
...
export const FadeInDiv = ({
    ...
    if (!activeTab || !(activeTab.content?.props?.items?.length > 0)) {
        return (
            <div className="flex flex-col items-center justify-center h-full gap-2 mt-4">
                <p>{t("no_data")}</p>
                <span
                    className="underline cursor-pointer"
                    onClick={(e) => handleOpenScreen(e, "/blog/new")}
                >
                    {t("become_blogger")}
                </span>
            </div>
        ...
        <AnimatePresence mode="popLayout">
            <div className="relative w-full h-full">
                {tabs.map((tab, idx) => {
                    ...
                    return (
                        <m.div key={tab.value} layoutId={tab.value}>
                            <m.div
                                style={{
                                    scale: 1 - idx * 0.1,
