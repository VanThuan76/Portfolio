...
const size = 15; // h-6
const boxClassName =
  "absolute bottom-0 h-5 w-5 rounded-sm bg-transparent p-px border border-black/30 border-box group ";
...
function Grid() {
      ...
      return (
        ...
        >
          <div
            ...
            className={cn(
              "h-full w-full scale-90 rounded bg-back/0 opacity-0 transition-all duration-700",
              {
  ...
  return (
    <div
      ref={containerRef}
      onClick={onMouseEnter}
      className={cn("absolute inset-0 h-full max-h-48 w-full", {
...
export default function InteractiveGrid({
  ...
  return (
    <div
      className={cn(
        "storybook-fix relative h-full w-full rounded-3xl",
        className,
      )}
    >
      <Grid />
