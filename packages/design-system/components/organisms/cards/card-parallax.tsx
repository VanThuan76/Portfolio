...
import { convertStringDay } from "@repo/design-system/helpers/convert-time";
...
const CardParallax = ({
  ...
  return (
    <div
      ref={container}
      className="min-h-[90vh] w-[80%] flex justify-center items-start sticky top-20 left-0"
    >
      <motion.div
      ...
      >
        <h2 className="m-0 text-2xl font-medium text-center uppercase">
          {title}
        </h2>
        <div className="flex h-full gap-12 mt-4">
        ...
        <motion.div
          ...
          style={{
            background: useMotionTemplate`radial-gradient(350px circle at ${mouseX}px ${mouseY}px,rgba(14, 165, 233, 0.15),transparent 80%)`,
          }}
        />
      </motion.div>
      <div
      ...
      <div
        style={{ marginTop: `${i * 20}px`, right: `-${(i + 1) * 40}px` }}
