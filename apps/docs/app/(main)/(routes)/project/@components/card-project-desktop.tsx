...
const CardProjectDesktop = ({ projects }: Props) => {
    ...
    return (
        <div className="relative hidden w-full h-[45rem] md:block border rounded-none shadow-none md:rounded-lg md:shadow-lg py-4 px-4 overflow-hidden">
            <div className="p-4">
                                        ...
                                        content: (
                                            <div className="flex flex-col items-start justify-start w-full h-full gap-2 mt-2">
                                                <div className="flex flex-wrap gap-2">
                                                    {project.tech_stack && project.tech_stack.split(",").map((item) => ({
                                                        title: item,
                                                        img: item && `/icon-techstack/${item.toLocaleLowerCase()}.svg`,
                                                    })).map((tech, i) => {
                                                        return (
                                                            <button key={i} type="button" className="relative inline-flex h-7 overflow-hidden rounded-md p-[1px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
