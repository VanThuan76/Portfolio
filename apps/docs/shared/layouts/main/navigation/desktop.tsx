...
export default function NavigationDesktop({
    ...
    const handleOpenScreen = useCallback(async (e: React.MouseEvent<HTMLDivElement>, href: string) => {
        e.preventDefault();
                    ...
                    <DockIcon key={item.label}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <div
                                    onClick={(e: React.MouseEvent<HTMLDivElement>) => handleOpenScreen(e, item.href)}
                                    className={cn(
                ...
                {Object.entries(DATA_DESKTOP_MENUS.main).map(([name, item]) => (
                    <DockIcon key={name}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <div
                                    onClick={(e: React.MouseEvent<HTMLDivElement>) => handleOpenScreen(e, item.url)}
                                    className={cn(
                                        buttonVariants({ variant: "ghost", size: "icon" }),
                                        "size-12 rounded-full relative",
